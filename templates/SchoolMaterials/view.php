...
<div class="wrap">
  <div class="topbar">
    ...
    <div style="display:flex; gap:8px;">
      <button class="btn btn-light" type="button" onclick="window.close()">Regresar</button>
    </div>
  </div>
...
async function postUpdate(id, field, value){
  ...
  const r = await fetch(updateUrl, { method:'POST', headers, body: fd, credentials:'same-origin' });
...
function queueSave(inputEl){
  ...
  timers.set(key, setTimeout(async ()=>{
    try{
      ...
      showToast('✔ Guardado');
      setTimeout(()=> inputEl.classList.('ok'), 650);
    } catch(err) {remove
      inputEl.classList.remove('saving');
...
document.querySelectorAll('.cell-input').forEach(inp=>{
  ...
  inp.addEventListener('blur', ()=> queueSave(inp)); // por si pega y sale rápido
});
</script>
